...
public class UserRepositoryImple implements UserRepository {
	...
	public int login(User user)
	{
		...
		User myuser = (User)jdbctemp.queryForObject(sql,new Object[] {user.getUserName(),user.getPassword()},new UserRowMapper());
		boolean test = false;		
				test = myuser.getPassword().equals(user.getPassword()) && myuser.getUserName().equals(user.getUserName());
	...
	public boolean register(User user) {
		if(user.getName().equals(null)||user.getPassword().equals(null)||user.getName().equals(null)|| user.getSalary()==0||user.getUserTypeId()==0)
			return false;
		
		String sql = "insert into user(user_name,password,name,salary,user_type_id)values(?,?,?,?,?)";
		int records = jdbctemp.update(sql,new Object[] {user.getUserName(),user.getPassword(),user.getName(),user.getSalary(),user.getUserTypeId()});
		if(records!=0)
			return true;
		else
			return false;
	...
	public void editUser(User myuser) {
		...
		jdbctemp.update(mysql,myuser.getName(),myuser.getPassword(),myuser.getName(),myuser.getSalary(),myuser.getUserTypeId(),myuser.getUserId());
	}
